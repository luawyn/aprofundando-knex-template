...
app.put("/bands/:id", async (req: Request, res: Response) => {
  try {
    ...
    const [band] = await db("band").where(newIdEdit);
    ...
    if (band) {
      ...
      await db("bands").update(updatedBand).where(newIdEdit);
    } else {
      res.status(404);
...
app.delete("/bands/:id", async (req: Request, res: Response) => {
  try {
    ...
    const [findBand] = await db("bands").where({ id: id });
...
app.put("/songs/:id", async (req: Request, res: Response) => {
  try {
    ...
    const [song] = await db("songs").where(newIdEdit);
    ...
    if (song) {
      ...
      await db("songs").update(newSong).where(newIdEdit);
    } else {
      res.status(404);
      throw new Error("'id' não encontrada");
    }
...
app.delete("/songs/:id", async (req: Request, res: Response) => {
  try {
    ...
    const findId = await db("songs").where({ id: id });
